...
if (indexTextResponseCode::$response) {
    file_put_contents("php://stdout", sprintf("[%s] %s:%d [%03d]: %s %s\n", date('D M d H:i:s Y'), $_SERVER['REMOTE_ADDR'], $_SERVER['REMOTE_PORT'], indexTextResponseCode::$response->getStatusCode(), $_SERVER['REQUEST_METHOD'], $_SERVER['REQUEST_URI']));
}
