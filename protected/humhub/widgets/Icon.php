...
use humhub\components\icon\IconProvider;
use Throwable;
use Yii;
...
class Icon extends Widget implements \Stringable
{
    ...
    /**
     * Resolves a semantic icon alias such as `edit` or `delete` to the icon name it stands for.
     *
     * The map is the `icon.alias` application parameter; a name it does not cover is returned
     * unchanged.
     *
     * @since 1.20
     */
    public static function resolveAlias(?string $name): ?string
    {
        ...
        return Yii::$app->params['icon']['alias'][$name] ?? $name;
    }
