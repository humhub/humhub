...
interface StateServiceInterface
{
    ...
    /**
     * @param null|array $config = ['filterContext' => 'name of the context']
     *
     * @return array|null
     */
     ...
     *
     * @param int|string|null $state
     * @param array $options Additional options depending on the implementation
     *
     * @return bool
     */
    ...
    /**
     * @param int[]|array|InvalidStateException|InvalidArgumentException $result Out parameter with the normalized
     *     $state, if the function returns true, or the error message otherwise.
     * @param int|string|array|null $state State to be validated. It's format depends on the setting of $allowArray:
     *        If the latter is set to false, then it can be either a single state (numeric value or its name) or an
     *     array with a single state as the key and array of conditions as the value. If $allowArray is set to true,
     *     then in addition to the above, the array is allowed to have multiple elements. However, in this case, all
     *     status must be given as array values. If a status is bound to a condition, both must be provided within an
     *     array with exactly one element having the status as key and an array of conditions as value.
