...
class ClassMap extends ActiveRecord
{
    ...
    /**
     * @param string|string[]|object $value Class name or names to be translated int their respective ID
     *
    ...
    public static function getIdByName($value)
    {
        ...
        return is_array($value)
            ? static::getIdByManyNames($value)
            : static::getIdByOneName($value);
    }
    ...
    public static function joinClassMap(
        ...
        if ($activeOnly) {
            $subQuery
                ->innerJoin(
                    $tableModuleEnabled,
                    "$tableClassMap.module_id = $tableModuleEnabled.module_id AND $tableModuleEnabled.is_suspended = 0"
                );
        }
