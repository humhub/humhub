    ...
    public static function getRegisteredModuleOverview(): object
    {
        ...
        foreach ($registeredModules as &$module) {
            try {
                $module = (object)[
                    ...
                    'isSuspended' => (bool)$module->is_suspended,
                ];
            } catch (UnknownPropertyException $t) {
                $module = (object)[
                    'moduleId' => $module->module_id,
                    'isSuspended' => false,
                ];
        ...
        $registeredModuleInfo = (object)[
            ...
            'enabled' => array_column(
                array_filter($registeredModules, static fn($module) => !$module->isSuspended),
                'moduleId'
            ),
            'disabled' => array_column(
                array_filter($registeredModules, static fn($module) => $module->isSuspended),
                'moduleId'
            ),
        ];
