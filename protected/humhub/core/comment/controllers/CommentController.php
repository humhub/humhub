...
namespace humhub\core\comment\controllers;
 ...
 * @since 0.5
 */
class CommentController extends \humhub\core\content\components\ContentAddonController
{
    ...
    public function actionShow()
    {
        ...
        $output = \humhub\core\comment\widgets\ShowMore::widget(['pagination' => $pagination, 'object' => $content]);
        foreach ($comments as $comment) {
            $output .= \humhub\core\comment\widgets\Comment::widget(['comment' => $comment]);
        }
    ...
    public function actionPost()
    {
        ...
        if ($message != "" && !Yii::$app->user->isGuest) {
            ...
            return $this->renderAjaxContent(
                            \humhub\core\comment\widgets\Comment::widget([
                                'comment' => $comment,
                                'justEdited' => true
                            ])
            );
    ...
    public function actionEdit()
    {
        ...
        if ($this->contentAddon->load(Yii::$app->request->post()) && $this->contentAddon->validate() && $this->contentAddon->save()) {
            ...
            return $this->renderAjaxContent(\humhub\core\comment\widgets\Comment::widget([
