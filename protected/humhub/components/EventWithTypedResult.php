...
class EventWithTypedResult extends Event
{
    ...
    public function setAllowedTypes(?array $allowedTypes): EventWithTypedResult
    {
        ...
        array_walk(
            ...
            static function (
                ...
                $self->addAllowedType($type, $options);
            },
            $this
        );
    ...
    public function getTypeDescription(
        ...
        switch ($type) {
            /** @noinspection PhpMissingBreakStatementInspection */ case 'int':
            $type = 'integer';
            case 'integer':
            case 'string':
            case 'float':
    ...
    public function validate(
        $value,
        $valueIfInvalid = InvalidArgumentTypeException::class
    ) {
     ...
     */
    protected static function _validateByType(
        array $types,
        &$value,
        &$valueIfInvalid,
        EventWithTypedResult $self
        ...
        array_walk(
            ...
            static function (
                    ...
                    if ($value === null && !$self->nullable) {
                        if ($valueIfInvalid === InvalidArgumentTypeException::class) {
                            throw new InvalidArgumentTypeException(
                                __METHOD__, [1 => 'value'], $self->allowedTypes, $value, false
                            );
                        }
                ...
                switch (strtolower($type)) {
                    case 'int':
                    case 'integer':
                        return static::validateInt($value, $options);

                    case 'string':
                        return static::validateString($value, $options);

                    case 'float':
                        return static::validateFloat($value, $options);

                    case 'array':
                        return static::validateArray($value, $options);

                    case 'object':
                        return static::validateObject($value, $options);

                    case 'null':
                        return static::validateNull($value, $options);
                }
        ...
        if (is_array($value)) {
            $isValid = array_reduce(
                ...
                static fn(
                    $isValid,
                    $item
                ): bool => $isValid && static::validateArray(
                        $item,
                        $options,
                        $strict,
                        $toArray,
    ...
    protected static function validateClass(
        ...
        if (!(($isClass = class_exists($type)) || ($isInterface = interface_exists($type)) || $isTrait = trait_exists(
                $type,
                true
            ))) {
            return false;
     ...
     * @noinspection PhpParameterByRefIsNotUsedAsReferenceInspection
     */
    protected static function validateFloat(
        &$value,
        &$options
    ): bool {
        // check if strict
     ...
     * @throws ArrayValueFound
     */
    protected static function validateNull(
        &$value,
        &$options
    ): bool {
        $strict = $options === 'strict' || $options === true || ($options = (array)$options && $options['strict'] ?? in_array(
     ...
     * @throws ArrayValueFound
     */
    protected static function validateObject(
        &$value,
        &$options,
        ?bool $strict = null,
        ?bool $toObject = null,
