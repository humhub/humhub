...
trait StatableActiveQueryTrait
{
    ...
    {
        // reset the states
        $this->whereDefaultFilter(['filterContext' => FilterableQueryInterface::FILTER_CONTEXT_INIT]);
    ...
    public function andWhereDefaultFilter(?array $config = null): self
    {
        ...
        /** @var StatableInterface $modelClass */
        $modelClass = $this->getModelClass();
        Helpers::checkClassType($modelClass, [StatableInterface::class]);
        $DefaultReturnedStates = $modelClass::getStateServiceTemplate()->getDefaultQueriedStates($config);
    ...
    public function prefixTableNameToUnqualifiedColumnNames(?string $tableName = null, ?Query $query = null, ?TableSchema $tableSchema = null)
    {
        /** @var StatableInterface $modelClass */
        $modelClass = $this->getModelClass();
        $tableName ??= $this->aliasFromSourceTables($tableName = $modelClass::tableName()) ?? $tableName;
        $query ??= $this;
        $tableSchema ??= $modelClass::getTableSchema();
