...
use humhub\libs\StatableQueryTrait;
...
abstract class StateService extends Component implements StateServiceInterface
{
    ...
    /**
     * @var array<string, array<int,int>|array> Can be a simple list of states, a complex where clause, or null for no default filtering
    ...
    public function initStates(): self
    {
        ...
        foreach ($this->defaultQueriedStates as $context => &$defaultQueriedStates) {
            if (!is_string($context)) {
                throw new InvalidConfigException(sprintf("Array keys for %s::%s must be context names (strings)", static::class, '$defaultQueriedStates'));
            }
    ...
    public function allowState(int $state, ?string $name = null): self
    {
        ...
        if ($name && $state !== (StatableInterface::RESERVED_STATE_NAMES[$name] ?? $state)) {
            throw new InvalidArgumentException(__METHOD__, [2 => '$name'], null, $name, "$name is a reserved status name. See StatableInterface::RESERVED_STATE_NAMES.");
        }
    ...
    public function getDefaultQueriedStates(?array $config = null): ?array
    {
        ...
        return $this->defaultQueriedStates[ $config['context'] ?? FilterableQueryInterface::FILTER_CONTEXT_DEFAULT] ?? null;
    }
    ...
    public function validateState(&$result, $state, ?array $params = [], bool $allowArray = true, bool $throwException = true): bool
    {
        ...
        if (!$allowArray && !$isSingleState && $countStates !== 1 && !is_array(reset($states))) {
            return $returnError(
                new InvalidArgumentTypeException(
                    ...
                    $state,
                    false,
                    'if parameter #3 ($allowArray) is true. State configuration: ' . Json::encode($states)
                )
            );
        }
        ...
        foreach ($states as $index => &$state) {
            ...
            if (null === $saveState = $this->isStateInList($state, $allowedStates)) {
                if ($condition === null && (is_string($index) || $countStates === 1) && null !== $saveState = $this->isStateInList($index, $allowedStates)) {
                    $condition = $state;
                    $state = $index;
                } else {
