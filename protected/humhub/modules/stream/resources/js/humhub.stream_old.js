...
humhub.module('stream', function (module, require, $) {
    ...
    /**
     * If a data-stream-topic is set on the stream root the stream will be initialized with a given topic
     * @type String
     */
    ...
    Stream.prototype.showLoader = function () {
        if(!this.$content.find('.loader').length) {
    ...
    Stream.prototype._load = function (cfg) {
        ...
        let data = {
            'StreamQuery[filters]': this.$.data('filters'),
        ...
        var $contentTypeFilter = $('#stream_filter_content_type');
        if($contentTypeFilter.length && $contentTypeFilter.val()) {
            data['StreamQuery[_includes]'] = $contentTypeFilter.val();
        }

        var that = this;
        return client.ajax(this.url, {data: data, beforeSend: function (xhr) {
            that.currentXhr = xhr;
    ...
    var init = function (pjax) {
        ...
        if (!pjax) {
            event.on('humhub:modules:content:newEntry.stream', function (evt, html) {
                ...
                getTopicPicker().select(topic.id, topic.name);
                ...
                getTopicPicker().remove(topic.id);
            }).on('humhub:topic:updated', function(evt, topics) {
                var stream = getStream();
                if(stream) {
                    // TODO add topics which are not already present in topic chooser
                    var topicPicker =  getTopicPicker();
        ...
        if (!stream) {
            ...
            _initFilterNav();
            if (stream.$.data(DATA_STREAM_TOPIC)) {
                topic.setTopics([stream.$.data(DATA_STREAM_TOPIC)]);
            }
        }
    };

    var getTopicPicker = function() {
        return Component.instance($('#stream_filter_topic'));
    };
    ...
    var focusTopicFilter = function(evt) {
        ...
        var topicPicker = Component.instance($('#stream_filter_content_type'));
        if(topicPicker) {
            topicPicker.focus();
        }
    };
