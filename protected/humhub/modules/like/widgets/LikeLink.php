    ...
    private function generateLikeTitleText($currentUserLiked, $likes)
    {
        ...
        if ($currentUserLiked == true) {
            ...
            if (count($likes) == 1) {
                ...
                $userlist = Yii::t('LileModule.base', 'You like this.');
            } else {
                // output, if more users like this
                $userlist .= Yii::t('LileModule.base', 'You'). "\n";
            }
        }
        ...
        for ($i = 0, $likesCount = count($likes); $i < $likesCount; $i++) {
            ...
            if ($likesCount == 1) {
                ...
                if ($likes[$i]->user->guid != Yii::$app->user->guid) {
                    ...
                    $userlist .= Html::encode($likes[$i]->user->displayName) . Yii::t('LileModule.base', ' likes this.');
                }
            } else {
                ...
                if ($i == $maxUser) {
                    ...
                    $userlist .= Yii::t('LileModule.base', 'and {count} more like this.', ['{count}' => (int)(count($likes) - $maxUser)]);
