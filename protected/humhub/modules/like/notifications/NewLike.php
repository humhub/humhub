...
class NewLike extends BaseNotification
{
    ...
    public function getMailSubject()
    {
        ...
        if ($this->groupCount > 1) {
            return Yii::t('LileModule.notifications', "{displayNames} likes your {contentTitle}.", [
                        'displayNames' => strip_tags($this->getGroupUserDisplayNames()),
                        'contentTitle' => $contentInfo
            ]);
        }

        return Yii::t('LileModule.notifications', "{displayName} likes your {contentTitle}.", [
    ...
    public function html()
    {
        ...
        if ($this->groupCount > 1) {
            return Yii::t('LileModule.notifications', "{displayNames} likes {contentTitle}.", [
                        'displayNames' => $this->getGroupUserDisplayNames(),
                        'contentTitle' => $contentInfo
            ]);
        }

        return Yii::t('LileModule.notifications', "{displayName} likes {contentTitle}.", [
