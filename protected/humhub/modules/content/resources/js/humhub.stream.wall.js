...
humhub.module('stream.wall', function (module, require, $) {
    ...
    WallStream.template = {
        loadSuppressedButton: '<div class="load-suppressed" style="display:none;"><a href="#" data-action-click="loadSuppressed" data-entry-key="{key}" data-entry-keys="{keys}" data-action-block="manual" data-ui-loader><i class="ti ti-chevron-down"></i>&nbsp;&nbsp;{message}&nbsp;&nbsp;<span class="badge text-bg-light">{contentName}</span></a></div>',
        updateBadge: '<div id="streamUpdateBadge" class="animated bounceIn"><span class="badge text-bg-info" style="cursor:pointer"><i class="ti ti-circle-arrow-up"></i> {text}</span></div>'
    };
    ...
    WallStreamFilter.template = {
        removeContentTypeLabel: '<a href="#" class="content-type-remove-label" data-action-click="removeContentTypeFilter" data-type-id="{id}"><span class="badge text-bg-secondary animated bounceIn"><i class="ti {image}"></i> {text}</span></a>'
    };
