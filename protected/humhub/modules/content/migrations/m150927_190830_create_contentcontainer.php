...
class m150927_190830_create_contentcontainer extends Migration
{
    ...
    public function up()
    {
        ...
        foreach ($spaces->each() as $space) {
            ...
            $this->updateSilent('space', ['contentcontainer_id' => Yii::$app->db->getLastInsertID()],
                'space.id=:spaceId', [':spaceId' => $space['id']]);
        }
        ...
        foreach ($users->each() as $user) {
            ...
            $this->updateSilent('user', ['contentcontainer_id' => Yii::$app->db->getLastInsertID()], 'user.id=:userId',
                [':userId' => $user['id']]);
        }
    }
