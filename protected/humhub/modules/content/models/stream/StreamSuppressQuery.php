 ...
 *
 * @see \humhub\modules\content\actions\Stream
 * @author luke
 * @since 1.2
 */
class StreamSuppressQuery extends StreamQuery
{
    ...
    protected function isSuppressionActive()
    {
        ...
        return !($this->preventSuppression || $this->limit <= $contentModule->streamSuppressLimit || $this->isSingleContentQuery());
    }
    ...
    protected function isSuppressed(&$results, $content)
    {
        ...
        if (in_array($content->object_model, array_merge($contentModule->streamSuppressQueryIgnore, $contentModule->defaultStreamSuppressQueryIgnore))) {
            return false;
        }
    ...
    public function getSuppressions()
    {
        ...
        foreach ($this->suppressions as $parentContentId => $infos) {
            ...
            $results[$parentContentId] = [
                ...
                'message' => Yii::t('ContentModule.base', 'Show {i} more.', ['i' => count($infos['contentIds'])]),
            ];
        }
