...
use humhub\modules\content\models\stream\filters\BlockedUsersStreamFilter;
...
use humhub\modules\content\models\stream\filters\DraftContentStreamFilter;
use humhub\modules\content\models\stream\filters\OriginatorStreamFilter;
use humhub\modules\content\models\stream\filters\ScheduledContentStreamFilter;
...
class StreamQuery extends Model
{
    ...
    protected function checkSort()
    {
        if (empty($this->sort) || !in_array($this->sort, [Stream::SORT_CREATED_AT, Stream::SORT_UPDATED_AT])) {
            $this->sort = Yii::$app->getModule('content')->settings->get('defaultStreamSort', Stream::SORT_CREATED_AT);
        }
    }
