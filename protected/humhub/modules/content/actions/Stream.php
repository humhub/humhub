...
use humhub\modules\content\events\StreamResponseEvent;
...
use humhub\modules\content\models\stream\WallStreamQuery;
...
abstract class Stream extends Action
{
    ...
    public function init()
    {
        ...
        if (!$this->isSingleContentRequest()) {
            $this->excludes = array_merge($this->excludes, Yii::$app->getModule('content')->streamExcludes);
        }
    ...
    private function handleEmptyResponse(StreamResponse $response)
    {
        if ($this->streamQuery->isSingleContentQuery()) {
            ...
            if (!$content) {
                $response->setError(400, Yii::t('ContentModule.base', 'The content could not be found.'));
            } elseif (!$content->canView()) {
                $response->setError(403, Yii::t('ContentModule.base', 'You are not allowed to view this content.'));
            }
        }
