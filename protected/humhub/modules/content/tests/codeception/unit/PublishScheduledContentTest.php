...
class PublishScheduledContentTest extends HumHubDbTestCase
{
    public function testPublishScheduledContent()
    {
        ...
        $this->assertEquals(StatableInterface::STATE_PUBLISHED, $postA->content->state);

        $postB = Post::findOne($postB->id);
        $this->assertEquals(StatableInterface::STATE_PUBLISHED, $postB->content->state);

        $postC = Post::findOne($postC->id);
        $this->assertEquals(StatableInterface::STATE_SCHEDULED, $postC->content->state);

        $postD = Post::findOne($postD->id);
        $this->assertEquals(StatableInterface::STATE_SCHEDULED, $postD->content->state);
    }
    ...
    private function createScheduledPost($date): Post
    {
        ...
        Yii::$app->request->setBodyParams([
            'state' => StatableInterface::STATE_SCHEDULED,
            'scheduledDate' => $datetime
        ]);

        $result = WallCreateContentForm::create($post, $space);
        $this->assertArrayHasKey('id', $result);
        $this->assertEquals(StatableInterface::STATE_SCHEDULED, $post->content->state);
