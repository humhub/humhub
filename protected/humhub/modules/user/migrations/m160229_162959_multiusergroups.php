...
class m160229_162959_multiusergroups extends \humhub\components\Migration
{
    ...
    public function up()
    {
        ...
        $this->execute('UPDATE group_user u SET is_group_admin = :value WHERE EXISTS (Select 1 FROM group_admin a WHERE u.user_id = a.user_id);',
            [':value' => 1]);
        ...
        foreach ($rows as $adminUserRow) {
            $this->insertSilent('group_user',
                ['user_id' => $adminUserRow['id'], 'group_id' => $adminGroupId, 'is_group_admin' => '1']);
        }
