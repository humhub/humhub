...
class ImageController extends BaseAccountController
{
    ...
    protected function getProfileImage($type)
    {
        if ($type == static::TYPE_PROFILE_IMAGE) {
            if (!$this->allowModifyProfileImage) {
                throw new HttpException(403, Yii::t('UserModule.controllers_ImageController', 'Access denied!'));
            }
            ...
            throw new HttpException(400, Yii::t('UserModule.controllers_ImageController', 'Invalid image type given!'));
        }
    }
    ...
    public function getUser()
    {
        if ($this->user === null && Yii::$app->request->get('userGuid') != '' && Yii::$app->user->getIdentity()->isSystemAdmin()) {
            ...
            if ($user === null) {
                throw new HttpException(404, Yii::t('UserModule.controllers_ImageController', 'Could not find user!'));
            }
            $this->user = $user;
        }
