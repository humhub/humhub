...
class AccountController extends BaseAccountController
{
    ...
    public function actionChangeUsername()
    {
        ...
        $model->scenario = \humhub\modules\user\models\forms\AccountChangeUsername::SCENARIO_CHANGE_USERNAME;

        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            // Check if username is in use, e.g. by other user
            $usernameAvailablyCheck = \humhub\modules\user\models\User::findOne(['username' => $model->username]);
