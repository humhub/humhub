...
class Membership extends ActiveRecord implements FindInstanceInterface
{
    ...
    public static function findInstance($identifier, array $config = []): ?self
    {
        if (is_array($identifier)) {
            ...
            $membership = Yii::$app->runtimeCache->getOrSet(static::class . "#$spaceId-$userId", fn() => Membership::findOne(['user_id' => $userId, 'space_id' => $spaceId]));

            if ($membership) {
                Yii::$app->runtimeCache->set(static::class . "#$membership->id", $membership);
            }
        } else {
            $membership = static::findInstanceHelper($identifier, $config);

            if ($membership) {
                Yii::$app->runtimeCache->set(static::class . "#$membership->space_id-$membership->user_id", $membership);
            }
        }
