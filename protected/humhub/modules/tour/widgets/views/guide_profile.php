...
<?php if (Yii::$app->user->isAdmin() == true) : ?>
            ...
            orphan: true,
                    ...
                    content: <?= json_encode(Yii::t('TourModule.widgets_views_guide_profile', 'You\'ve completed the user profile guide!<br><br>To carry on with the administration guide, click here:<br /><br />')); ?> + "<a href='javascript:gotoAdministration = true; tourCompleted();'><?= Yii::t("TourModule.widgets_views_guide_profile", "<strong>Administration (Modules)</strong>"); ?></a><br><br>"
            }
<?php else : ?>
            {
            orphan: true,
...
<?php endif; ?>
    ...
    function tourCompleted() {
        // load user spaces
        $.ajax({
            'url': '<?= Url::to(['/tour/tour/tour-completed', 'section' => 'profile']); ?>',
            'cache': false,
            ...
            if (gotoAdministration == true) {
                ...
                window.location.href = "<?= Url::to(['/admin/module/list-online', 'tour' => 'true']); ?>";
            } else {
                // redirect to dashboard
                window.location.href = "<?= Url::to(['/dashboard/dashboard']); ?>";
            }
