...
<script>
    ...
    $(document).ready(function () {
        ...
        spacesTour.addSteps([
            ...
            },
            {
                ...
                content: <?= json_encode(Yii::t('TourModule.widgets_views_guide_spaces', "That's it for the space guide.<br><br>To carry on with the user profile guide, click here: ")); ?> + "<a href='javascript:gotoProfile = true; tourCompleted();'><?= Yii::t("TourModule.widgets_views_guide_spaces", "<strong>Profile Guide</strong>"); ?></a><br><br>"
            }
        ]);
    ...
    function tourCompleted() {
        // load user spaces
        $.ajax({
            'url': '<?= Url::to(['/tour/tour/tour-completed', 'section' => 'spaces']); ?>',
            'cache': false,
            ...
            if (gotoProfile == true) {
                // redirect to profile
                window.location.href = "<?= Yii::$app->user->getIdentity()->createUrl('/user/profile', ['tour' => true]); ?>";
            } else {
                // redirect to dashboard
                window.location.href = "<?= Url::to(['/dashboard/dashboard']); ?>";
            }
