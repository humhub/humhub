...
foreach (explode(",", $currentValue) as $guid) {
    ...
    if ($space != null) {
        ...
        $name = $space->name;
        $newValue .= '<li class="spaceInput" id="' . $space->guid . '"><img class="img-rounded" alt="24x24" data-src="holder.js/24x24" style="width: 24px; height: 24px;" src="' . $imageUrl . '" alt="' . addslashes($name) . 'r" width="24" height="24">' . addslashes($name) . '<i class="fa fa-times-circle"></i></li>';
    }
}
?>
