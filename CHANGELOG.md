...
1.19 (TBD)
----------
- Enh: Added a mail (SMTP) delivery configuration step to the web installer (after the basic step) reusing the admin mailing settings form, with an inline "Test" button that verifies the entered settings via AJAX; configuring mail never blocks completion, the step is skipped on automated/managed-hosting installs (fixed mail config), and the `php` transport is not offered under Docker (`HUMHUB_DOCKER`)
