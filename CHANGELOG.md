...
1.19 (TBD)
----------
- Fix: Module acceptance tests could redirect every request to the installer ("Login[username]" not found / setup wizard) although the site was installed — `SettingFixture` (an `ActiveFixture`) writes the `setting` table via raw SQL, bypassing the `SettingsManager` cache, so the fixture-loaded base settings (including the `InstallationState`) did not take effect and a reader sharing the settings cache (the acceptance test server) kept serving the stale, un-installed state; `SettingFixture` now reloads the settings on load/unload
