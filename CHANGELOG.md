...
1.19 (TBD)
----------
- Fix #8276: Profile images intermittently returned 404 after being replaced from a console/queue context (e.g. a module syncing remote avatars) — the `AssetManager` publish-state cache was collected per request and only written back at the end of web requests, so an unpublish in a console process deleted the published file but left the stale cache entry behind, and subsequent web requests kept rendering the dead URL without republishing. Publish states are now stored as individual cache entries that are written and invalidated immediately in all contexts (this also removes the read-modify-write race between concurrent web requests and replaces the `AssetImage`/`AssetImageRegistry` publish caching layers, including `TrackableArray`). The publish-state cache is optional (`AssetManager::$cachePublishState`) and auto-enabled only when a non-local mount is configured — with local mounts (the default) every publish verifies the published copy against the filesystem and self-heals, and published asset image URLs derive their cache-busting `?t=` from the file's modification time instead of the request time
