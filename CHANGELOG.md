...
- Fix #8358: On an instance with public self-registration disabled (`auth.anonymousRegistration = 0`) but invite-by-e-mail or invite-by-link enabled, opening a valid invitation link rendered the registration page without the actual form fields — `RegistrationController` gated `showRegistrationForm` on `AuthController::isSelfRegistrationFormVisible()`, which also requires self-registration to be globally enabled; a validated invite token/link is now an independent authorization and no longer subject to that global setting (new `AuthController::isRegistrationFormEnabled()`, which only reflects `Module::$showRegistrationForm`)
