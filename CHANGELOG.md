...
1.19 (TBD)
----------
- Enh: Add the `UnreadCountChangedEvent`, triggered whenever a user's unseen notification count changes or by modules such as Messenger
...
- Fix #8318: On the permalink view of a sub comment, the root comment list ended without a "Show next" pagination link, hiding all root comments after the highlighted comment's parent — the list is anchored around the parent comment, but the pagination condition in `comments.php` checked the highlight id, which is `null` on the root level when the highlighted comment lives on the nested level (regression from #7917); additionally, a comment list anchored by a permalink now stays focused around the anchored comment (a `commentsPreviewMax` window of previous comments with a "Show previous" link) instead of inlining up to the full view-mode limit (25) of previous comments above it without any pagination
...
- Fix #8318: The "Show previous/next {count} comments" pagination links showed at most one page size (e.g. "Show previous 5 comments" with 15 comments left) — `ShowMore` now counts all remaining comments in the respective direction, so the label reflects the real rest and counts down while loading; the pagination links are also centered now, in line with the upcoming comment UI design. A single comment beyond the visible range — on the initial rendering, around a permalinked comment or when loading more — is now included directly instead of being hidden behind a "Show previous/next 1 comments" link (#8320)
- Fix #8318: The "Show next {count} comments" pagination link overlapped the comment above it — the separator inside the `.showMore` box was unintentionally hidden by the CSS rule that hides the leading separator of a comment list (`:first-of-type` also matched the first `hr` inside the box); the rule is now scoped to the list level, and the leftover (always hidden) separator inside the blocked-comment view was removed
