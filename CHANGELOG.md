...
- Enh #8519: Rebuilt the spaces directory (`/spaces`) as a Vue island on the core card directory kit, fed by `GET /api/v2/space` — now the platform's space search (`SpaceListQuery`, with `purpose` for directory, chooser and pickers) — plus `space/states` and the new `space/<id>/follow`. Cards load page by page with skeletons and show banner, member and follower counts, tags and the membership and follow buttons; the space header's `FollowButton` is an island too. The directory widgets, `SpaceDirectoryQuery`, `SpaceDirectoryFilters`, `space/spaces/load-more` and the web actions `space/space/follow|unfollow` are removed; modules move to `SpaceListQuery::EVENT_INIT` and `SpaceDirectoryFilterSet::EVENT_INIT`, and old `keyword`/`connection` links are redirected, see the migration guide
