...
- Enh #8367: An admin impersonating a user had access to everything the impersonated user can see, including their private content — private content is now hidden and private spaces are no longer accessible while impersonating. The behavior is configurable with the new `\humhub\modules\admin\Module::$impersonateMode`; the `*_LOGGED` modes write a warning naming the impersonating and the impersonated user to the log on each impersonation. Added `Yii::$app->user->isPrivateContentRestricted` and the `ControllerAccess::RULE_PRIVATE_CONTENT_ACCESS` access rule, which modules can use to deny actions exposing private content
