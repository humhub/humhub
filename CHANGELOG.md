...
1.19.0-beta.2 (TBD)
-------------------
- Fix #8379: The membership and friendship buttons emitted their full option array — titles, urls and arbitrary HTML attributes — as `data-button-options` and merged it back into the widget unfiltered when the button was re-rendered after an AJAX action, so a request could control the rendered markup; the client now only passes the name of a server side option set (`data-button-variant`), see `MembershipButton::registerVariant()`
