...
1.20.0 (Unreleased)
-------------------
- Enh #8521: Added an item browser kit to the Vue component set — `ViewSwitch`, `PathBar`, `TileGrid`, `SelectionMenu`, `DropZone` and `ProgressFrame`, after the design system v2's file browser (`resources/scss/_item-browser.scss`), for module views that browse a collection level by level with selection and drag & drop; file payloads carry the Tabler name of their file-type icon as `icon` (`MimeHelper::getIconNameByExtension()`)
- Fix #8521: A `DropdownMenu` toggle that was only ever plain-clicked — never opened through the component's own `open()` — leaked its Bootstrap dropdown instance on unmount: only the instance the component itself creates was disposed, while Bootstrap's own data-api handler lazily creates and keeps its own instance for a plain-clicked toggle. That instance is now disposed too
- Enh #8519: Rebuilt the spaces directory (`/spaces`) as a Vue island on the core card directory kit, fed by `GET /api/v2/space` — now the platform's space search (`SpaceList`, with `purpose` for directory, chooser and pickers) — plus `space/states` and the new `space/<id>/follow`. Cards load page by page with skeletons and show banner, member and follower counts, tags and the membership and follow buttons; the space header's `FollowButton` is an island too. The directory widgets, `SpaceDirectoryQuery`, `SpaceDirectoryFilters`, `space/spaces/load-more` and the web actions `space/space/follow|unfollow` are removed; modules move to `SpaceList::EVENT_INIT` (one filter object) and `SpaceList::EVENT_BUILD`, and old `keyword`/`connection` links are redirected, see the migration guide
- Fix #8517: An `/api/v2` controller without any authentication method - one not allowing session authentication, on an installation where no module contributes a token method, or one whose session opt-in is misspelled - ran every action as a guest, even with guest access disabled, because Yii's `CompositeAuth` skips authentication when it has no method. Such a controller now answers with a 401 (guest-allowed actions aside) through the new `humhub\components\api\ApiAuth`, and logs a warning naming the controller
- Enh #8515: Rebuilt the marketplace as a Vue island on a new `/api/v2/marketplace` API. The page no longer waits for humhub.com: it renders at once with skeleton cards, and modules, categories and the version notice load afterwards, page by page. The groups are gone — one grid, modules with an available update first; installing, updating (one or all), enabling, the settings and licence keys run through the API and Vue dialogs, `POST /api/v2/module/<id>/enable` enables a module. Across `/api/v2`, the caller's relationships (like, space membership, friendship) are now set with `PUT` and operations are verb sub-paths taking `POST` (`marketplace/module/<id>/install`, `…/update`, `module/<id>/enable`), see `docs/develop/concept-api.md`. The page toolbar, filter bar and card grid are a reusable core kit (`PageToolbar`, `FilterBar`, `CardDirectory`) for People, Spaces and module pages later; lists and their filters are one server-side model (`humhub\components\listing`: `FilterableList`, `ListFilter`, ready filters) from which the API validates and applies and the pages get their filter definitions. The marketplace widgets, `FilterService`, the `marketplace` JavaScript module and the old install/update/purchase routes are removed, and the page's `keyword`/`tags` URL parameters are now `q`/`tag` (old `keyword` links are redirected); modules extending the marketplace filters add a filter on `ModuleList::EVENT_INIT`, see the migration guide
- Enh #8504: Replaced Font Awesome 4 with Tabler Icons. `humhub\widgets\Icon` renders `<i class="ti ti-<name>">` through the new `TablerIconProvider`, Tabler names are the canonical icon names from now on, and `Icon::getNames()` returns them from the installed package. Font Awesome is no longer shipped; raw `fa fa-<name>` markup in modules, themes and translations keeps rendering through `css/icon-legacy.css`, and Font Awesome names passed to `Icon::get()`, menu entries or stored in the database are translated by `LegacyIconMap` — both stay until 1.22. A name both libraries know renders the Tabler icon, which is outline where Font Awesome was solid. `FontAwesomeAsset`, `FontAwesomeIconProvider`, `Icon::$names` and the unused list-item and border options are deprecated and go in 1.21. Themes and custom SCSS styling `.fa` on core-rendered icons have to switch to `.ti`; the `IconPicker` hands the icon list to the browser instead of rendering thousands of options; see the migration guide. `Button` (and everything built on it — `Link`, `MenuLink`, the left navigation menus) now puts a space between an icon and its label instead of concatenating them directly: Font Awesome 4's fixed per-character advance width used to leave enough room on its own, Tabler's tightly-cropped glyphs do not. The left navigation menus (e.g. the admin menu) also lost their oversized 15px horizontal inset, cut to 8px in `.list-group-item`, now that the smaller Tabler glyphs no longer needed the extra room
- Enh #8500: The theming component no longer uses the settings table as a per-variable cache. The `theme` setting holds the theme name instead of an absolute base path, and everything derived from it — the resolved path, the parent chain and the SCSS variables — moves into a single `theme.state` row, replacing `themeParents` and the roughly 150 `theme.var.*` rows per theme, which were never cleaned up when a theme was switched. The state is validated against the theme name, the system revision, the custom SCSS and the modification time of every `scss/variables.scss` in the theme tree, so a moved installation, a deleted theme directory, an edited theme, changed design settings, a toggled module and a core update all invalidate it without an additional cache lookup. The admin design settings no longer rebuild the variables on every render of the page, which cost roughly 600 queries per page view
- Enh #8483: Moved the local configuration directory from `protected/config` to `config` in the installation root, beside the `.env` file. The old directory is still read, with the new one merged on top, so installations keep working and can be moved one file at a time — but `@config` follows the new directory, so translation overrides under `@config/messages` and view overrides under `@config/views` stop being applied where they are, which is the one part that fails silently. A new Prerequisites entry lists what is left behind; the dashboard warning follows in 1.21. `dynamic.php` is moved by a migration, and until then keeps being written where it already is. None of the local configuration files is shipped any more — the new directory holds `common.example.php`, `web.example.php` and `console.example.php`; see the upgrade notes
- Enh #8479: Dissolved the `stream` module into the `content` module — the actions land in `humhub\modules\content\actions`, the queries and filters in `humhub\modules\content\models\stream`, the two widgets beside the stream entry widgets that were already there, and the module class is gone. The old class names stay as deprecated shims until 1.21, but `Yii::$app->getModule('stream')` cannot be shimmed: its four properties are configured on `content` now, and the `stream.defaultSort` setting becomes `content.defaultStreamSort`. A theme overriding one of the two moved widget views has to move the override along, which is the one part that fails silently; see the migration guide
- Fix #8479: An event a module registered on a deprecated shim class silently stopped firing — `Event::trigger()` walks the parents of the sender upwards, and a shim is a subclass of the class the core instantiates, so the handler was never reached. `ModuleManager` now redirects such a registration to the replacement class and logs a warning naming the module. This affected every `config.php` naming a class moved out of the `ui` module in this cycle
...
- Enh #8403: The control links of the content menu (`EditLink`, `DeleteLink`, `PermaLink`, … and `ContentTopicButton`) are `MenuLink`s instead of widgets and describe themselves to the `/api/v2/content/<id>/controls` endpoint; `WallEntryControlLink` is deprecated, `DescribableWidget` removed
