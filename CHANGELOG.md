...
- Fix: A `DropdownMenu` toggle that was only ever plain-clicked — never opened through the component's own `open()` — leaked its Bootstrap dropdown instance on unmount: only the instance the component itself creates was disposed, while Bootstrap's own data-api handler lazily creates and keeps its own instance for a plain-clicked toggle. That instance is now disposed too
