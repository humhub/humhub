...
1.19 (TBD)
----------
- Fix: Comments inserted via AJAX (a newly posted comment, "Show previous/next comments", the comments popup) were missing the separator line and spacing towards the preceding comment — most visibly a new comment sticking directly to the highlighted comment on a comment permalink view; the BS5 refactoring (#7902) moved the separator out of the comment view into the list views, so the `comment/post` and `comment/show` responses no longer contained it (regression from #7902)
- Fix: On the permalink view of a sub comment, the root comment list ended without a "Show next" pagination link, hiding all root comments after the highlighted comment's parent — the list is anchored around the parent comment, but the pagination condition in `comments.php` checked the highlight id, which is `null` on the root level when the highlighted comment lives on the nested level (regression from #7917); additionally, a comment list anchored by a permalink now stays focused around the anchored comment (a `commentsPreviewMax` window of previous comments with a "Show previous" link) instead of inlining up to the full view-mode limit (25) of previous comments above it without any pagination
- Fix: The "Show next {count} comments" pagination link overlapped the comment above it — the separator inside the `.showMore` box was unintentionally hidden by the CSS rule that hides the leading separator of a comment list (`:first-of-type` also matched the first `hr` inside the box); the rule is now scoped to the list level, and the leftover (always hidden) separator inside the blocked-comment view was removed
