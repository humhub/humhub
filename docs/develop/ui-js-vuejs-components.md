...
- **`StatusBar`** (`<status-bar>`, `protected/humhub/vue/StatusBar.vue`) — the platform's user-feedback bar, and the first **infrastructure island**: nothing renders it with props and no other component nests it. `humhub\widgets\StatusBar` (a `LayoutAddons` widget, so it exists on every full page) mounts it once as `<status-bar id="status-bar">`, and every message reaches it through the bridge instead of through props — see [Legacy interop: status messages](ui-js-vuejs-interop.md#status-messages) for the queue that makes a message triggered before the island mounts work. A 1:1 replacement of the jQuery bar it succeeds: one message at a time, the same `.status-bar-body`/`.status-bar-content` markup and level classes, the same auto-close timings, the same details toggle for error traces. The 500 ms slide moved from jQuery `animate()` to a CSS transition in `_user-feedback.scss` — the component only toggles `status-bar-visible`.
